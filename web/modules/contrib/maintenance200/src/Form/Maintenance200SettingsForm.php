  ...
  public function validateForm(array &$form, FormStateInterface $form_state) {
    ...
    if ($form_state->getValue('maintenance200_status_code')) {
      ...
      if (!array_key_exists($form_state->getValue('maintenance200_status_code'), $http_status_codes)) {
        $form_state->setErrorByName('maintenance200_status_code', $this->t('A valid HTTP status code is required.'));
      }
    }
    else {
      $form_state->setErrorByName('maintenance200_status_code', $this->t('You must provide an HTTP status code.'));
    }
  }
