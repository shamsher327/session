    ...
    attach: function (context) {
      $('.paragraphs-add-dialog--categorized', context).each(function () {
        ...
        $toggle.once().on('click', function () {
          var $self = $(this);
          var $parent = $self.closest('.paragraphs-add-dialog--categorized');

          if ($self.hasClass('style-list')) {
            $parent.addClass('paragraphs-style-list');
          }
          else {
            $parent.removeClass('paragraphs-style-list');
          }
        });
      });
    }
    ...
    attach: function (context) {
      $('.paragraphs-add-dialog--categorized', context).each(function () {
        ...
        $filter.once().on('input', function () {
          var $self = $(this);
          var $parent = $self.closest('.paragraphs-add-dialog--categorized');
          var $search_description = $('.search-description :checkbox', $parent);
          filterItems($parent, $self.val(), $search_description.is(':checked'));
        });
        var $search_description = $('.search-description :checkbox', $filter_wrapper);
        $search_description.once().on('change', function () {
          var $self = $(this);
          var $parent = $self.closest('.paragraphs-add-dialog--categorized');
          var $filter = $('.item-filter', $parent);
          filterItems($parent, $filter.val(), $self.is(':checked'));
        });
      });
    }
  };
